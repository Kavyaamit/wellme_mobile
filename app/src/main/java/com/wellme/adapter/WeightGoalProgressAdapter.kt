...
class WeightGoalProgressAdapter(val context: Context, val list : List<UserWeightDTO>) : RecyclerView.Adapter<WeightGoalProgressAdapter.ViewHolder>(){
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view : View = LayoutInflater.from(context).inflate(R.layout.item_weight_goal, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        var dto : UserWeightDTO = list.get(position)
        ...
        if(dto!=null){
            ...
            if(!UtilMethod.instance.isStringNullOrNot(s_weight)){
                ...
                var index : Int = dto.added_on.lastIndexOf(".")
                holder.tv_weight_amount.setText(UtilMethod.instance.getFormatedAmountString(f_weight)+" "+context.resources.getString(R.string.kg))
                if(index>-1){
                    holder.tv_weight_date.setText(UtilMethod.instance.getDate(dto.added_on.substring(0, index)))
                }
                else{
                    holder.tv_weight_date.setText(dto.added_on)
                }
