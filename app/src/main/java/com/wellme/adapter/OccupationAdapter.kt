...
class OccupationAdapter(var context: Context, var occupation_list : List<OccupationDTO>,var occupation_filterlist : List<OccupationDTO>, var onItemClickCallback: OnItemClickListener.OnItemClickCallback, var pos : Int,var occupation_id : String) : RecyclerView.Adapter<OccupationAdapter.ViewHolder>() {

    var orig: MutableList<OccupationDTO> = ArrayList<OccupationDTO>()
    ...
    override fun getItemCount(): Int {
        return occupation_filterlist.size
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        var dto : OccupationDTO = occupation_filterlist.get(position)
        var regular : Typeface? = ResourcesCompat.getFont(context, R.font.poppins_regular)
        holder.tv_title!!.setTypeface(regular)
        holder.tv_title!!.text = dto.occupations


            if(occupation_id.equals(dto.id)){
