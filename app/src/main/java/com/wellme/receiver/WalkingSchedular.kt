...
class WalkingSchedular : JobService(){
    ...
    override fun onStartJob(params: JobParameters?): Boolean {
        ...
        if(!UtilMethod.instance.isStringNullOrNot(s_reminder)) {
            ...
            if(reminder_old_list!=null){
                reminderDTO = reminder_old_list.get(0)
                if(reminderDTO!=null){
        ...
        if(!UtilMethod.instance.isStringNullOrNot(s_time)){
            ...
            var componentName : ComponentName = ComponentName(applicationContext, WalkingSchedular::class.java)
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                createNotification("Walking")
            }
            else {

                sendNotification(Intent(), "Walking")
            }
        }
    ...
    fun createNotification(title: String) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            builder.setContentTitle("Walking") // required
                .setSmallIcon(R.drawable.logo) // required
                .setContentText("Please start walking for fitness") // required
                .setDefaults(Notification.DEFAULT_ALL)
                .setAutoCancel(true)
                .setContentIntent(pendingIntent)
                .setTicker("Please start walking for fitness")
    ...
    fun sendNotification(
        ...
        try {
            notificationBuilder.setContentTitle("Walking")
            notificationBuilder.setContentText("Please start walking for fitness")
            //             notificationBuilder.setLargeIcon(bitmap);
        } catch (e: Exception) {
            e.printStackTrace()
        }
        ...
        notificationBuilder.setStyle(NotificationCompat.BigTextStyle().bigText("Please start walking for fitness"))
