...
class LogWeightSchedular : JobService(){
    ...
    override fun onStartJob(params: JobParameters?): Boolean {
        ...
        if(!UtilMethod.instance.isStringNullOrNot(s_reminder)) {
            ...
            if(reminder_old_list!=null){
                reminderDTO = reminder_old_list.get(2)
                if(reminderDTO!=null){
        ...
        if(!UtilMethod.instance.isStringNullOrNot(s_time)){
            ...
            if(s_repeating_time.equals("day")){
                var max_day : Int = calendar!!.getActualMaximum(Calendar.DAY_OF_MONTH)
                diff =  max_day * 24 * 60 * 60 * 1000L
            }
            ...
            if(diff>0) {
                var componentName: ComponentName =
                    ComponentName(applicationContext, LogWeightSchedular::class.java)
    ...
    fun createNotification() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            builder.setContentTitle("Log Weight") // required
                .setSmallIcon(R.drawable.logo) // required
                .setContentText("Please update your weight in log") // required
                .setDefaults(Notification.DEFAULT_ALL)
                .setAutoCancel(true)
                .setContentIntent(pendingIntent)
                .setTicker("Please update your weight in log")
    ...
    fun sendNotification(
        ...
        try {
            notificationBuilder.setContentTitle("Log Weight")
            notificationBuilder.setContentText("Please update your weight in log")
            //             notificationBuilder.setLargeIcon(bitmap);
        } catch (e: Exception) {
            e.printStackTrace()
        }
        ...
        notificationBuilder.setStyle(NotificationCompat.BigTextStyle().bigText("Please update your weight in log"))
