...
class TrackMealSchedular : JobService() {
    ...
    override fun onStartJob(params: JobParameters?): Boolean {
        ...
        if (!UtilMethod.instance.isStringNullOrNot(s_reminder)) {
            ...
            if (reminder_old_list != null) {
                reminderDTO = reminder_old_list.get(3)
                if (reminderDTO != null) {
        ...
        if (!UtilMethod.instance.isStringNullOrNot(s_time)) {
            ...
            if (s_repeating_time.equals("day")) {
                ...
                var list: List<String> = s_time!!.split(",")
                var l1: Long = 0L
                ...
                if (list != null) {
                    ...
                    for (i in 0..list.size - 1) {
                        var date: Date? = UtilMethod.instance.getDateWithoutDate1(list.get(i))
                        if (date != null) {
                            Log.v("Position ", "==> " + i)
                    ...
                    if (current_ms < calendar_ms) {
                        difference_ms = calendar_ms - current_ms
                    }

                }

                Log.v("Difference ", "==> " + diff)
                if (diff > 0) {
                    var componentName: ComponentName =
                        ComponentName(applicationContext, TrackMealSchedular::class.java)
    ...
    fun createNotification(title: String) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            builder.setContentTitle("Meal") // required
                .setSmallIcon(R.drawable.logo) // required
                .setContentText("It's time to take some food") // required
                .setDefaults(Notification.DEFAULT_ALL)
                .setAutoCancel(true)
                .setContentIntent(pendingIntent)
                .setTicker("It's time to take some food")
    ...
    fun sendNotification(
        ...
        try {
            notificationBuilder.setContentTitle("Meal")
            notificationBuilder.setContentText("It's time to take some food")
        ...
        notificationBuilder.setStyle(NotificationCompat.BigTextStyle().bigText("It's time to take some food"))
